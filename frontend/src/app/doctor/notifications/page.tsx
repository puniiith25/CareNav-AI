...
import { Bell, ShieldCheck, CheckCircle2, Clock, FileText, Calendar } from "lucide-react";
...
export default function DoctorNotificationsPage() {
  ...
  useEffect(() => {
    async function fetchNotifs() {
      try {
        const data = await api<any[]>("/api/doctor/notifications");
      ...
      <div className="space-y-6 max-w-4xl">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            Realtime updates for booked appointments, consent grants, report uploads, and follow-ups.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {notifications.map((n) => (
              ...
              >
                <div className="w-9 h-9 rounded-full bg-teal-950 border border-teal-800 text-teal-400 flex items-center justify-center shrink-0 mt-0.5">
