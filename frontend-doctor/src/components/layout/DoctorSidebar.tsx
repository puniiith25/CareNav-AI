...
export function DoctorSidebar() {
  ...
  const navItems = [
    ...
    { label: "Patient Review", href: "/consultations", icon: Stethoscope },
  ...
  return (
    <aside className="hidden md:flex flex-col w-60 bg-[#0f172a] text-slate-200 border-r border-slate-800 h-screen sticky top-0 shrink-0 select-none z-30">
      {/* Brand */}
      <div className="p-5 border-b border-slate-800/80">
        <Link href="/dashboard" className="flex items-center gap-2.5 group">
          <div className="w-9 h-9 rounded-xl bg-teal-600 text-white flex items-center justify-center shadow-md shadow-teal-900/30 group-hover:scale-105 transition-transform">
            <Stethoscope className="w-5 h-5" />
          </div>
          <div>
            <div className="font-bold text-sm tracking-tight text-white flex items-center gap-1.5">
              CareNav <span className="text-teal-400 text-[10px] font-bold px-1.5 py-0.5 rounded bg-teal-950/80 border border-teal-800/60">MD</span>
            </div>
            <div className="text-[10px] text-slate-400 font-medium">Doctor Portal</div>
          </div>
        </Link>
      </div>
      ...
      <div className="p-3 border-t border-slate-800/80">
        <div className="flex items-center justify-between p-2 rounded-xl bg-slate-900/60 border border-slate-800">
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-8 h-8 rounded-full bg-teal-700 text-white flex items-center justify-center text-xs font-bold shrink-0">AS</div>
